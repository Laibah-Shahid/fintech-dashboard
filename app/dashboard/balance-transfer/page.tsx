...
export default function BalanceTransfer() {
  ...
  const onSubmit = async (values: TransferFormValues) => {
    ...
    try {
      ...
      form.reset();
      const updatedAccounts = await fetch("/api/accounts").then(res => res.json());
  ...
  return (
    <DashboardLayout>
      <div className="mb-8">
        ...
        <p className="text-gray-400">View your account balances and transfer funds between accounts.</p>
      </div>
          ...
          accounts.map((account) => (
            <Card key={account.id} className="bg-secondary/20">
              <CardHeader>
                ...
                <CardDescription>Account: •••• {account.accountNumber.slice(-4)}</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  <div>
                    <div className="text-sm text-gray-400">Available Balance</div>
                    <div className="text-3xl font-bold">{formatCurrency(account.balance, account.currency)}</div>
                  </div>
      ...
      <Card className="bg-secondary/20 mb-8">
        <CardHeader>
          ...
          <CardDescription>Move money between your accounts in the sandbox</CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <Select onValueChange={field.onChange} defaultValue={field.value} disabled={transferring}>
                          <FormControl>
      ...
      <Card className="bg-fintech-purple/10 border-fintech-purple/30">
        ...
        <CardContent>
          <ul className="space-y-3">
            <li className="flex">
              <ArrowRight className="h-5 w-5 text-fintech-purple mr-2" />
